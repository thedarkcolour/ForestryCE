...
public class TraderMenu extends TileMenu<TileTrader> {
	public static TraderMenu fromNetwork(int windowId, Inventory inv, FriendlyByteBuf data) {
		TileTrader tile = TileUtil.getTile(inv.player.level(), data.readBlockPos(), TileTrader.class);
		return new TraderMenu(windowId, inv, tile);
	}

	public TraderMenu(int windowId, Inventory inv, TileTrader tile) {
