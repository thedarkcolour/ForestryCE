...
public class BeeExploreParticle extends TextureSheetParticle {
	...
	public void tick() {
		...
		if (this.age < this.lifetime * 0.25) {
			// venture out
			this.xd *= 0.92 + 0.3D * this.random.nextFloat();
			this.yd = (this.yd + 0.3 * (-0.5 + this.random.nextFloat())) / 2;
			this.zd *= 0.92 + 0.3D * this.random.nextFloat();
		} else if (this.age < this.lifetime * 0.5) {
			// slow down
			this.xd *= 0.75 + 0.3D * this.random.nextFloat();
			this.yd = (this.yd + 0.3 * (-0.5 + this.random.nextFloat())) / 2;
			this.zd *= 0.75 + 0.3D * this.random.nextFloat();
