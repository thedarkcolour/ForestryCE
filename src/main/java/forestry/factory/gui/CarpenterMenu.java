...
public class CarpenterMenu extends LiquidTanksMenu<TileCarpenter> implements IContainerCrafting {
	private ItemStack oldCraftPreview = ItemStack.EMPTY;

	public static CarpenterMenu fromNetwork(int windowId, Inventory inv, FriendlyByteBuf data) {
		TileCarpenter tile = TileUtil.getTile(inv.player.level(), data.readBlockPos(), TileCarpenter.class);
		return new CarpenterMenu(windowId, inv, tile);
	}

	public CarpenterMenu(int windowId, Inventory inventoryplayer, TileCarpenter tile) {
