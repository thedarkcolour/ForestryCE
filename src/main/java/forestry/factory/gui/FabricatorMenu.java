...
public class FabricatorMenu extends LiquidTanksMenu<TileFabricator> implements IContainerCrafting {
	public static FabricatorMenu fromNetwork(int windowId, Inventory inv, FriendlyByteBuf data) {
		TileFabricator tile = TileUtil.getTile(inv.player.level(), data.readBlockPos(), TileFabricator.class);
		return new FabricatorMenu(windowId, inv, tile);
	}

	public FabricatorMenu(int windowId, Inventory playerInventory, TileFabricator tile) {
