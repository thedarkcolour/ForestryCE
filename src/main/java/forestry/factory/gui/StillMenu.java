...
public class StillMenu extends LiquidTanksMenu<TileStill> {
	public static StillMenu fromNetwork(int windowId, Inventory inv, FriendlyByteBuf data) {
		TileStill tile = TileUtil.getTile(inv.player.level(), data.readBlockPos(), TileStill.class);
		return new StillMenu(windowId, inv, tile);
	}

	public StillMenu(int windowId, Inventory player, TileStill tile) {
