...
public class BottlerMenu extends LiquidTanksMenu<TileBottler> {
	public static BottlerMenu fromNetwork(int windowId, Inventory inv, FriendlyByteBuf data) {
		TileBottler tile = TileUtil.getTile(inv.player.level(), data.readBlockPos(), TileBottler.class);
		return new BottlerMenu(windowId, inv, tile);
	}

	public BottlerMenu(int windowId, Inventory player, TileBottler tile) {
