...
public class MoistenerMenu extends LiquidTanksMenu<TileMoistener> implements ISlotChangeWatcher {
	public static MoistenerMenu fromNetwork(int windowId, Inventory inv, FriendlyByteBuf data) {
		TileMoistener tile = TileUtil.getTile(inv.player.level(), data.readBlockPos(), TileMoistener.class);
		return new MoistenerMenu(windowId, inv, tile);
	}

	public MoistenerMenu(int windowId, Inventory player, TileMoistener tile) {
