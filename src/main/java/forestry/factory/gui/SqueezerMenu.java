...
public class SqueezerMenu extends LiquidTanksSocketedMenu<TileSqueezer> {
	public static SqueezerMenu fromNetwork(int windowId, Inventory inv, FriendlyByteBuf data) {
		TileSqueezer tile = TileUtil.getTile(inv.player.level(), data.readBlockPos(), TileSqueezer.class);
		return new SqueezerMenu(windowId, inv, tile);
	}

	public SqueezerMenu(int windowId, Inventory player, TileSqueezer tile) {
