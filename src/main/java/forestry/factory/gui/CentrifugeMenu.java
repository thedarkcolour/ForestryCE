...
public class CentrifugeMenu extends SocketedMenu<TileCentrifuge> {
	private ItemStack oldCraftPreview = ItemStack.EMPTY;

	public static CentrifugeMenu fromNetwork(int windowId, Inventory inv, FriendlyByteBuf data) {
		TileCentrifuge tile = TileUtil.getTile(inv.player.level(), data.readBlockPos(), TileCentrifuge.class);
		return new CentrifugeMenu(windowId, inv, tile);
	}

	public CentrifugeMenu(int windowId, Inventory player, TileCentrifuge tile) {
