...
public class RaintankMenu extends LiquidTanksMenu<TileRaintank> {
	public static RaintankMenu fromNetwork(int windowId, Inventory inv, FriendlyByteBuf data) {
		TileRaintank tile = TileUtil.getTile(inv.player.level(), data.readBlockPos(), TileRaintank.class);
		return new RaintankMenu(windowId, inv, tile);
	}

	public RaintankMenu(int windowId, Inventory player, TileRaintank tile) {
