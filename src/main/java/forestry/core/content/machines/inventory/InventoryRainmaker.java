...
public class InventoryRainmaker extends InventoryAdapterTile<TileMillRainmaker> {
	...
	public boolean canSlotAccept(int slotIndex, ItemStack stack) {
		if (slotIndex == SLOT_SUBSTRATE) {
			RainmakerFuel substrate = stack.getItemHolder().getData(ForestryDataMaps.RAINMAKER_FUELS);
	...
	public void setItem(int slotIndex, ItemStack itemStack) {
		if (slotIndex == SLOT_SUBSTRATE) {
			RainmakerFuel substrate = itemStack.getItemHolder().getData(ForestryDataMaps.RAINMAKER_FUELS);
			if (substrate != null) {
