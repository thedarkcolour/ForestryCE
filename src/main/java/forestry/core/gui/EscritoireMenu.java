...
public class EscritoireMenu extends TileMenu<TileEscritoire> implements IGuiSelectable {
	...
	public EscritoireMenu(int id, Inventory playerInv, TileEscritoire tile) {
	...
	public static EscritoireMenu fromNetwork(int windowId, Inventory playerInv, FriendlyByteBuf extraData) {
		TileEscritoire tile = TileUtil.getTile(playerInv.player.level(), extraData.readBlockPos(), TileEscritoire.class);
		return new EscritoireMenu(windowId, playerInv, tile);
	}
	...
	public void handleSelectionRequest(ServerPlayer player, int primary, int secondary) {
		...
		if (primary == -1) {
			...
			tile.choose(new ResolvableProfile(player.getGameProfile()), primary);
		}
	}
}
