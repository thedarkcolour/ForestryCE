...
public abstract class TileMenu<T extends BlockEntity> extends ForestryMenu {
	...
	protected TileMenu(int windowId, MenuType<?> type, Inventory playerInventory, T tile, int xInv, int yInv) {
		super(type, windowId, playerInventory.player);
		addPlayerInventory(playerInventory, xInv, yInv);
		this.tile = tile;
	}

	@Deprecated(forRemoval = true)
	protected TileMenu(int windowId, MenuType<?> type, T tile) {
		super(type, windowId, null);
		this.tile = tile;
	}

	protected TileMenu(int windowId, MenuType<?> type, T tile, Player player) {
	...
	public void broadcastChanges() {
		...
		if (this.tile instanceof TilePowered tilePowered) {
			...
			int workCounter = tilePowered.getStepCounter();
			if (workCounter != this.previousWorkCounter) {
				guiNeedsUpdate = true;
				this.previousWorkCounter = workCounter;
			}

			int ticksPerWorkCycle = tilePowered.getStepsPerWorkCycle();
