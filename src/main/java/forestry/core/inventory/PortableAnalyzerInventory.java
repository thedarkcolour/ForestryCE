...
public class PortableAnalyzerInventory extends ItemInventory implements IErrorSource {
	...
	public PortableAnalyzerInventory(Player player, ItemStack stack) {
		super(7, stack);
		this.player = player;
	}

	public static boolean isAnalyzingFuel(ItemStack stack) {
		if (stack.isEmpty()) {
			return false;
		}
	...
	private void analyzeSpecimen(ItemStack specimen) {
		...
		if (individual != null) {
			if (!individual.isAnalyzed()) {
				// Requires energy
				if (!isAnalyzingFuel(getItem(SLOT_ENERGY))) {
					return;
				}
	...
	public Set<IError> getErrors() {
		...
		if (specimen.isEmpty()) {
			...
			if (IIndividualHandlerItem.isIndividual(specimen) && !isAnalyzingFuel(getItem(SLOT_ENERGY))) {
